...
%         end
        ...
        transmitter_action_matrix = (floor(gama*jammer_next_action_selection)+1):10; 
        
        transmitter_action_selection = transmitter_action_matrix(randi(length(transmitter_action_matrix)));
%          end
        
%         transmitter_action_record(episode,step) = jammer_next_action_selection;
...
% 
 transmitter_utility_averag = smooth(transmitter_utility_average,40);
% jammer_utility_averag = smooth(jammer_utility_average,30,'loess');
 transmitter_rate_averag = smooth(transmitter_rate_average,40);
figure(1);
plot(transmitter_utility_averag,'linewidth',1);
xlabel('time slot');
ylabel('Utility');
...
figure(2);
plot(transmitter_rate_averag,'linewidth',1);
xlabel('time slot');
ylabel('Data rate');
legend('PHC','Q-learning','RCRA');
grid on;
...
% w = 16:16:96;
figure(3);
plot(N_antenna,sum_rate_PHC,'-d','linewidth',1);
...
figure(4);
plot(N_antenna,sum_utility_PHC,'-d','linewidth',1);
xlabel('The number of the antenna');
ylabel('Utility');
legend('PHC','Q-learning','RCRA');
grid on;
